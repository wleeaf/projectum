...
class MonthGrid(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._hover_day = None           # day cell under the cursor (hover feedback)
    ...
    def _paint_cells(self, p: QPainter, cw: float, ch: float) -> None:
        ...
        for week in range(6):
            for col in range(7):
                ...
                is_weekend = col >= 5
                is_hover = (d == self._hover_day and self._drag is None
                            and self._day_sel is None)
                cell = self._cell_rect(week, col).adjusted(1.5, 1.5, -1.5, -1.5)
                ...
                if is_hover:
                    # Lift the cell and accent its border so days read as clickable.
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self._day_sel is not None and (event.buttons() & Qt.MouseButton.LeftButton):
            ...
            event.accept()
            return
        # Plain hover: highlight the day under the cursor + pick a cursor.
        if self._drag is None and self._day_sel is None:
            pos = event.position()
            day = self._day_at(pos)
            ...
            if bar is not None:
                ...
                cur = Qt.CursorShape.PointingHandCursor   # days are clickable
            else:
...
class LinksDialog(QWidget):
    ...
    def showEvent(self, event) -> None:
        ...
        self._search.setFocus()   # type to relate immediately

    def _add_top_result(self) -> None:
        if self._results.count():
            self._add_from_result(self._results.item(0))
